...
const ChartManager = {
    ...
    createOverviewQuantityChart(categoryStats) {
        ...
        const labels = sortedCategories.map(c => c.name);
        // Làm tròn số gói
        const quantities = sortedCategories.map(c => Math.round(Utils.safeNumber(c.totalGoi)));
        const colors = sortedCategories.map(c => c.color || '#667eea');
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: (context) => {
                                ...
                                const casesRounded = Math.round(category.cases);
                                const goiRounded = Math.round(context.raw);
                                return [
                                    `Số lượng: ${Utils.formatNumber(casesRounded)} thùng`,
                                    `Số lượng: ${Utils.formatNumber(goiRounded)} gói`
                                ];
                            }
                        }
                    },
                    ...
                    datalabels: {
                        ...
                        formatter: (value, context) => {
                            ...
                            // Làm tròn số thùng và số gói
                            const casesRounded = Math.round(category.cases);
                            const goiRounded = Math.round(value);
                            const total = context.chart.data.datasets[0].data.reduce((a, b) => a + b, 0);
                            const percentage = total > 0 ? ((goiRounded / total) * 100).toFixed(1) : 0;
                            return `${Utils.formatNumber(casesRounded)} thùng\n(${Utils.formatNumber(goiRounded)} gói - ${percentage}%)`;
                        },
                        font: {
                            weight: 'bold',
                            size: 11
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: (context) => {
                                ...
                                // Làm tròn số thùng và số gói
                                const casesRounded = Math.round(product.cases);
                                const goiRounded = Math.round(product.totalGoi);
                                
                                if (product.kv) {
                                    return [
                                        `Doanh thu: ${Utils.formatCurrency(context.raw)}`,
                                        `Mã SP: ${product.ma_sp}`,
                                        `Số lượng: ${Utils.formatNumber(casesRounded)} thùng (${Utils.formatNumber(goiRounded)} gói)`,
                                        `KV: ${product.kv}`
                                    ];
                                } else {
                                    return [
    ...
    createDetailQuantityChart(categoryName, products) {
        ...
        // Làm tròn số gói
        const quantities = sortedProducts.map(p => Math.round(Utils.safeNumber(p.totalGoi)));
